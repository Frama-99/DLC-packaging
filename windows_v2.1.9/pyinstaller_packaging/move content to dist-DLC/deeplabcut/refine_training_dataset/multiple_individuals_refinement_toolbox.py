...
class ImagePanel(wx.Panel):
    ...
    def drawplot(self,img,img_name,itr,index,threshold,cmap,preview,keep_view=False):
...
#        self.axes.clear()
        ...
        self.figure,self.axes,self.canvas,self.toolbar,self.ax = self.image_panel.drawplot(self.img,img_name,self.iter,self.index,self.threshold,self.colormap,self.preview,keep_view=True)
...
#        dlg = wx.FileDialog(self, "Choose the machinelabels file for current iteration.",cwd, "",wildcard=fname,style=wx.FD_OPEN | wx.FD_FILE_MUST_EXIST)
        ...
        if os.path.isfile(self.dataname):
            ...
            self.choiceBox,self.slider,self.checkBox,self.visualization_rdb = self.choice_panel.addCheckBoxSlider(self.bodyparts,self.file,self.markerSize)
            ...
            self.figure,self.axes,self.canvas,self.toolbar,self.ax = self.image_panel.drawplot(self.img,img_name,self.iter,self.index,self.threshold,self.colormap,self.preview)
            self.axes.callbacks.connect('xlim_changed', self.onZoom)
            self.axes.callbacks.connect('ylim_changed', self.onZoom)
    ...
    def force_outside_labels_Nans(self,index,ind,bodyparts):
        ...
        for bpindex, bp in enumerate(bodyparts):
            testCondition = self.Dataframe.loc[index,(self.scorer,ind,bp,'x')] > self.width or self.Dataframe.loc[index,(self.scorer,ind,bp,'x')] < 0 or self.Dataframe.loc[index,(self.scorer,ind,bp,'y')] > self.height or self.Dataframe.loc[index,(self.scorer,ind,bp,'y')] <0
            if testCondition:
    ...
    def check_labels(self):
        ...
        for i in self.Dataframe.index:
            ...
            for ind in self.individual_names:
                if ind =='single':
                    self.Dataframe = MainFrame.force_outside_labels_Nans(self,i,ind,self.uniquebodyparts)
                else:
                    self.Dataframe = MainFrame.force_outside_labels_Nans(self,i,ind,self.multianimalbodyparts)
        return(self.Dataframe)
...
# ###########################################################################
    def saveEachImage(self):
        ...
        for bpindex, bp in enumerate(self.updatedCoords):
            self.Dataframe.loc[self.Dataframe.index[self.iter]][self.scorer,bp[0][-1], bp[0][-3],'x' ] = self.updatedCoords[bpindex][-1][0]
            self.Dataframe.loc[self.Dataframe.index[self.iter]][self.scorer,bp[0][-1], bp[0][-3],'y' ] = self.updatedCoords[bpindex][-1][1]
    ...
    def getLabels(self,img_index,ind,bodyparts):
        ...
        for bpindex, bp in enumerate(bodyparts):
            image_points = [[self.Dataframe[self.scorer][ind][bp]['x'].values[self.iter],self.Dataframe[self.scorer][ind][bp]['y'].values[self.iter],bp,bpindex,ind]]
